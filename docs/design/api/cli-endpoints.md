# REST API Endpoint for CLI tool
...
API tokens can be created and managed through the developer console:

```text
https://console.rylees.ai/settings/token
```
...
|----------|----------|-------------|
| GET | `/projects/{uuid}` | Retrieve project information, including project context and LLM configuration. |
| POST | `/projects/{uuid}` | Publish a generated release note to the project's release history. |
