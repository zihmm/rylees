...
The platform uses four DNS records for `rylees.ai`:

| DNS record | Type | Target | Purpose |
| ---------- | ---- | ------ | ------- |
| `rylees.ai` | A / AAAA | Web app | Redirects to Developer Console |
...
Specific DNS records take precedence over the wildcard, so `api` and `console` are unaffected. The web server uses four Apache virtual hosts:

- `rylees.ai` (bare root) → 301 redirect to `console.rylees.ai`
- `api.rylees.ai` → Laravel API (`public/index.php`)
- `console.rylees.ai` → serves the Developer Console SPA
- `*.rylees.ai` (wildcard, after exact `api` / `console` / bare-root hosts) → serves the public Release History SPA

The wildcard TLS certificate covers customer subdomains. `rylees.ai`, `api.rylees.ai`, and `console.rylees.ai` use their own certificates.

No DNS API calls are made when a customer is created — the `organisations.slug` is generated by the application, and the wildcard DNS + Apache configuration automatically makes `{slug}.rylees.ai` resolvable without any additional provisioning step.
